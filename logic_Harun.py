...
"""
from efesdevelope import article_exists, find_product_by_id   #importiert die Funktion article_exists und find_product_by_id aus der Datei efesdevelope.py, um sie in diesem Skript verwenden zu können
...
def decrease_stock(products, product_id, amount):
    ...
    product = find_product_by_id(products, product_id)              #sucht das Produkt mit der angegebenen ID
    if product == None:
...
def get_statistics(products):
    """Gibt Statistiken über die Produkte zurück."""
    total_products = len(products)                                  #berechnet die Gesamtzahl der Produkte
    total_stock = sum(product['stock'] for product in products)     #berechnet die Gesamtmenge aller Lagerbestände
    low_stock_count = len(get_low_stock_products(products))         #berechnet die Anzahl der Artikel mit niedrigem Lagerbestand
    return {
